  ...
  return <aside className="space-y-4 lg:sticky lg:top-[76px] lg:max-h-[calc(100vh-92px)] lg:overflow-auto">
    ...
    <section className="overflow-hidden rounded-xl border border-parchment-600/25 bg-parchment-900/35">
      ...
      <div className="p-3 text-sm">
        {tab === 'people' && <div className="space-y-3">{save.state.characters.length ? save.state.characters.map((c) => <div key={c.id} className="rounded-lg border border-parchment-600/20 bg-ink/20 p-3"><div className="flex items-center justify-between"><span className="text-gold-light">{c.name}</span><span className="text-[10px] text-parchment-200/45">{c.status}</span></div><div className="mt-1 text-xs text-parchment-200/60">{c.role || c.description || '尚无详细资料'}</div>{c.knownFacts.length > 0 && <div className="mt-2 space-y-0.5 text-[11px] text-parchment-200/50">{c.knownFacts.slice(-3).map((fact, i) => <div key={`${c.id}-f-${i}`}>· {fact}</div>)}</div>}</div>) : <p className="text-xs text-parchment-200/45">故事人物将在正文中出现。</p>}</div>}
        {tab === 'world' && <div className="space-y-3">{save.state.currentScene && <div><div className="text-xs text-parchment-200/45">当前场景</div><div className="mt-1 text-gold-light">{save.state.currentScene.name}</div><p className="mt-1 text-xs leading-relaxed text-parchment-200/65">{save.state.currentScene.description}</p><div className="mt-2 text-[11px] text-parchment-200/45">{[save.state.currentScene.time, save.state.currentScene.weather].filter(Boolean).join(' · ')}</div></div>}<div className="border-t border-parchment-600/15 pt-3"><div className="text-xs text-parchment-200/45">最新进展</div><p className="mt-1 text-xs leading-relaxed text-parchment-200/70">{save.state.latestProgress || save.state.summary || '故事刚刚开始。'}</p></div></div>}
        {tab === 'items' && <div className="space-y-2">{save.state.inventory.length ? save.state.inventory.map((item) => <div key={item.id} className="flex items-start justify-between rounded-lg border border-parchment-600/20 p-2.5"><div><div className="text-parchment-100">{item.name}</div><div className="text-[11px] text-parchment-200/50">{item.description || item.kind}</div></div><span className="text-gold-light">×{item.quantity}</span></div>) : <p className="text-xs text-parchment-200/45">暂无物品。</p>}</div>}
    ...
    <div className="rounded-xl border border-parchment-600/20 bg-ink/20 p-3 text-xs text-parchment-200/50">故事脉动：{save.state.randomEvent.pending ? '新的事件正在靠近' : '命运沿当前路径流动'}</div>
  </aside>;
}
