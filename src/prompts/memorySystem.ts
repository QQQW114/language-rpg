 ...
 * - system：故事守护者身份、长期一致性记忆整理规则与主角视角约束。
 * - user：buildMemoryUser 拼装周期性记忆更新任务。
 * - 输入包含：故事大纲、世界书硬设定、上一版长期记忆、最近对话、本轮决策/状态提取、当前人物档案、当前能力、当前场景、玩家标记。
 * - 输入包含：目标最大长度 maxChars，并要求输出更新后的长期一致性记忆正文。
 * - chat + 司书库启用时，服务层还会追加司书库 systemRules / manifest，并开放对应工具。
 * - 输出：纯文本长期记忆块，供后续故事、规划与审校模型引用。
 */
export const MEMORY_SYSTEM = `你是这段互动小说的"故事守护者"。你会严格参照用户消息中的故事大纲、世界书、上一版长期记忆、最近对话、状态提取、人物档案、能力、当前场景和玩家标记，把后续故事写手最容易忘记的稳定事实整理成短而可用的记忆块。
...
整理原则：
1. 只保留会影响后续连续性的事实：人物外观/服装/习惯/称呼/关系变化、主角承诺或计划、未解线索、能力状态、地点状态、时间安排、重要情绪债。
...
5. 玩家通过【玩家标记的关键记忆】明确标记的内容必须保留——除非它本身已被推翻或自然解决——并优先以"主角承诺/计划/未解线索"形式落入条目，永不静默舍弃。
